...
(ert-deftest filetags-should-add-tags-also-to-origin-file-through-link
    ()
  (let ((path "/tmp/filetags-test/")
    ...
    (make-symbolic-link (concat path filename)
                        (concat path linkname)
                        t)
    (filetags-update-tags-write (concat path linkname)
                                tags-with-prefix)
    (should (equal (file-truename (concat path "link -- tag.txt")) (concat path "test -- tag.txt")))
    (delete-directory path t)))

(ert-deftest filetags-should-add-tags-also-to-origin-file-and-to-intermediate-link-through-double-link
    ()
  (let ((path "/tmp/filetags-test/")
        (filename "test.txt")
        (linkname1 "link.txt")
    ...
    (f-touch (concat path filename))
    (make-symbolic-link (concat path filename)
                        (concat path linkname1)
                        t)
    (make-symbolic-link (concat path linkname1)
                        (concat path linkname2)
                        t)
    (filetags-update-tags-write (concat path linkname2)
                                tags-with-prefix)
    (should (equal (file-truename (concat path "anotherlink -- tag.txt")) (concat path "test -- tag.txt")))
    (should (equal (file-truename (concat path "link -- tag.txt")) (concat path "test -- tag.txt")))
    (should (equal (file-chase-links (concat path "anotherlink -- tag.txt")
                                     1) (concat path "link -- tag.txt")))
    (delete-directory path t)))

(ert-deftest filetags-can-read-in-controlled-vocabulary-from-file-test
    ()
  (let ((path "/tmp/filetags-test/")
        (filename "filetags")
        (tag-lines '("test1" "test2")))
    (delete-directory path t)
    (make-directory path t)
...
(ert-deftest filetags-can-read-in-controlled-vocabulary-from-file-with-comments-test
  ...
  (let ((path "/tmp/filetags-test/")
        (filename "filetags")
        (tag-lines '("test1 #comment" "#comment" "test2")))
    (delete-directory path t)
    (make-directory path t)
    (create-filetags-file (concat path filename)
                          tag-lines)
...
(ert-deftest filetags-can-read-in-controlled-vocabulary-from-file-with-mutually-exclusive-tags-test
    ()
  (let ((path "/tmp/filetags-test/")
        (filename "filetags")
        (tag-lines '("summer winter" "test1 test2")))
    (delete-directory path t)
    (make-directory path t)
    (create-filetags-file (concat path filename)
                          tag-lines)
